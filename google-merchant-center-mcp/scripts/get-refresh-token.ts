#!/usr/bin/env tsx
/**
 * One-time helper to generate an OAuth2 refresh token for google-merchant-center-mcp.
 *
 ...
 *   npm run auth
 *
 * Requires GOOGLE_MERCHANT_CENTER_CLIENT_ID and GOOGLE_MERCHANT_CENTER_CLIENT_SECRET to already be set in .env
 * (create the OAuth 2.0 "Desktop app" client in Google Cloud Console first — you can reuse the
 * same client you made for the other google-*-mcp servers or Gmail).
 */
...
const clientId = process.env.GOOGLE_MERCHANT_CENTER_CLIENT_ID;
const clientSecret = process.env.GOOGLE_MERCHANT_CENTER_CLIENT_SECRET;

if (!clientId || !clientSecret) {
  console.error(
    "\nMissing GOOGLE_MERCHANT_CENTER_CLIENT_ID / GOOGLE_MERCHANT_CENTER_CLIENT_SECRET.\n" +
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    console.log(`GOOGLE_MERCHANT_CENTER_REFRESH_TOKEN=${tokens.refresh_token}\n`);
  } catch (err) {
...
server.listen(PORT, () => {
  ...
  console.log("(Use the Google account that has access to your Merchant Center account.)");
