#!/usr/bin/env tsx
/**
 * One-time helper to generate a Google Analytics (GA4) OAuth2 refresh token.
 *
 ...
 *   npm run auth
 *
 * Requires GOOGLE_ANALYTICS_CLIENT_ID and GOOGLE_ANALYTICS_CLIENT_SECRET to already be set
 * in .env (create the OAuth 2.0 "Desktop app" client in Google Cloud Console first — you can
 * reuse the same client you made for google-ads-mcp or Gmail).
 */
...
if (!clientId || !clientSecret) {
  console.error(
    "\nMissing GOOGLE_ANALYTICS_CLIENT_ID / GOOGLE_ANALYTICS_CLIENT_SECRET.\n" +
...
server.listen(PORT, () => {
  ...
  console.log("(Use the Google account that has access to your GA4 property.)");
