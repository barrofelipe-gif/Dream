#!/usr/bin/env tsx
/**
 * One-time helper to generate a Google Ads API OAuth2 refresh token.
 *
 ...
 *   npm run auth
 *
 * Requires GOOGLE_ADS_CLIENT_ID and GOOGLE_ADS_CLIENT_SECRET to already be set in .env
 * (create the OAuth 2.0 "Desktop app" client in Google Cloud Console first).
 */
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    const tokens = (await tokenResponse.json()) as { refresh_token?: string; error?: string; error_description?: string };
...
server.listen(PORT, () => {
  ...
  console.log("(Use the Google account that has access to your Google Ads account.)");
  console.log(
    "\nNote: as of August 2026, Google requires a passkey to complete this consent flow. " +
      "If prompted, follow Google's on-screen passkey setup — it's a one-time step.\n"
  );
